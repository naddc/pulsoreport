...
cargar_datos <- function(params) {
  ...
  for (nombre in nombres_validos) {
    ...
    if (file.exists(filepath)) {
      ...
      message(paste("Archivo no encontrado en", filepath, "- Buscando en paquete 'pulso2'..."))
      filepath_pkg <- system.file(file.path('extdata', params[[nombre]]), package = "pulso2")

      if (file.exists(filepath_pkg) && nzchar(filepath_pkg)) {
        message(paste("Cargando", nombre, "desde el paquete 'pulso2' en", filepath_pkg))
