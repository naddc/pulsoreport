...
#' @export
plot_torta <- function(data, var, title = NULL, title.pos = 'top', legend.pos = 'bottom', show_n = FALSE) {
  ...
  if (title.pos == 'top') {
    p <- p +
      ggplot2::labs(title = stringr::str_wrap(if (is.null(title)) attributes(data[[rlang::as_name(rlang::enquo(var))]])$label
                                              else title, width = 30)) +
